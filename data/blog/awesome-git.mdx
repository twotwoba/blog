...
相较于 `SourceTree` 之类的可视化图形 git 管理软件, 我个人更喜欢使用简单的命令行去处理 git 相关的工作，原因也很简单：

1. 装逼嘛🐶（开个玩笑～）
2. 熟悉各个命令，我会很清楚各个文件、分支、commit的状态，比图形化的东西让我更加安心
3. 配置好下文的各个命令别名，你切换软件功夫，我可能已经完成对应的工作😁

> 平时 vscode 的 `Source Control` 我用的也很多，比如把文件从 Staged 状态回到 Modified 状态。有一说一，这个比敲命令快而且便捷很多！

---

## 配置别名

`.zshrc` 中默认配置了 `git` 插件，内置了很多别名，但是并不适合我，别名当然要做适合自己的！

```sh:配置别名前，先在.zshrc中配置下git的别名
# vim ~/.zshrc
alias g=git
```
...
git config alias.ck checkout                # 动起来～
...
git config alias.rb 'branch -m'             # 改
