---
title: '基于 Promise A+ 实现一个 Promise'
summary: '前端不能没有 Promise，就像西方不能没有耶路撒冷！Promise 是每一个前端er都必须精通的API，一起来看看PromiseA+的实现吧'
date: '2022-09-29'
tags: ['javascript']
images: []
---

如果你有 CET4 的水平，请一定先看完这个👉 [Promise A+ github](https://github.com/promises-aplus/promises-spec) ，而不是其他杂文（包括这篇 👀）

> 特别提醒：Promise A+ 主要专注于可交互的 then 方法

## 解读规范

### promise state

三种状态：` pending`、`fulfilled`、`rejected`。`pending ` 最终转为 `fulfilled`或 `rejected` 且是不可逆的。
