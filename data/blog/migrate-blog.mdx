...
再回到 cloudfare 的 `DNS->记录` 下, 点击添加记录, 把 `www` 和 `你的域名` 分别添加进去, 剩下的就是等待了, 一般一个小时以内就可以使用你的域名进行流畅地访问了🎉.

![cloudfare cname](/static/images/cloudfare-cname.png)
